...
function replyInviteLgpd(memberName: string): string {
  return (
    `👋 Olá, *${memberName}*! Você foi convidado(a) para o Vesta da sua família.\n\n` +
    "📋 *Aviso de privacidade (LGPD):*\n" +
    "Para vincular seu WhatsApp coletarei apenas seu nome. " +
    "Seus dados são usados exclusivamente para enviar resumos e avisos da família.\n\n" +
    "Responda *ACEITO* para continuar."
  );
}

function replyInviteAskName(suggestedName: string): string {
  return (
    `✨ Ótimo! Como quer ser chamado(a)?\n\n` +
    `_Sugestão: ${suggestedName}_\n\n` +
    "Responda com seu nome ou *OK* para usar a sugestão acima."
  );
}

function replyInviteComplete(name: string, magicToken: string, domain: string | null): string {
  const appUrl = domain ? `https://${domain}/app?magic=${magicToken}` : null;
  const linkLine = appUrl ? `\n\n🔗 Acesse o Vesta no navegador (válido por 30 min):\n${appUrl}` : "";
  return (
    `✅ Pronto, *${name}*! Seu WhatsApp foi vinculado ao Vesta da família.\n\n` +
    `Você passará a receber o resumo diário e avisos importantes por aqui.${linkLine}\n\n` +
    `_Dica: responda PAUSAR para silenciar por 24h._`
  );
}
...
// ── Member WA invite redemption ───────────────────────────────────────────────

/**
 * Returns true for VESTA-XXXXXX tokens that contain at least one letter.
 * These are member invite tokens generated by the API (hex charset).
 * Pure-digit VESTA-\d{6} tokens are handled by the onboarding token store
 * and never reach handleWaOnboarding.
 */
function looksLikeMemberInviteToken(text: string): boolean {
  const upper = text.trim().toUpperCase();
  if (!/^VESTA-[A-Z0-9]{6}$/.test(upper)) return false;
  return /[A-Z]/.test(upper.slice(6));
}

/**
 * Step 1 of abbreviated invite onboarding.
 * Token received from an unknown sender → validate (read-only), then open a
 * session at INVITE_LGPD so the invitee goes through LGPD consent + name
 * confirmation before being linked.
 */
async function startMemberInviteFlow(
  phoneRaw: string,
  phoneNorm: string,
  token: string,
  log: Logger,
  ...
  // Read-only lookup — do not mark used_at yet (atomic claim happens at INVITE_NAME).
  const [invite] = await db
    .select()
    .from(memberInvitesTable)
    .where(and(
      eq(memberInvitesTable.token, normalizedToken),
      isNull(memberInvitesTable.used_at),
      gt(memberInvitesTable.expires_at, new Date()),
    ))
    .limit(1);

  if (!invite) {
    log.warn({ phone: phoneNorm, tokenPrefix: normalizedToken.slice(0, 9) }, "wa-invite: token not found or expired");
    return {
      kind: "wa_onboarding",
      phone: phoneRaw,
      reply:
        "⚠️ Código de convite inválido ou expirado.\n\n" +
        "Peça ao administrador da família que gere um novo convite.",
    };
  }
  ...
  const [member] = await db
    .select()
    .from(membersTable)
    .where(eq(membersTable.id, invite.member_id))
    .limit(1);

  if (!member) {
    log.error({ tokenPrefix: normalizedToken.slice(0, 9), memberId: invite.member_id }, "wa-invite: member not found");
    return {
      kind: "wa_onboarding",
      phone: phoneRaw,
      reply: "⚠️ Código de convite inválido. O membro não foi encontrado.",
    };
  }

  // Create or replace a session in INVITE_LGPD so further messages are routed correctly.
  ...
  await db
    ...
    .values({
      phone: phoneNorm,
      step: "INVITE_LGPD",
      data: { invite_token: normalizedToken, name: member.name },
      lgpd_accepted: false,
      expires_at: sessionExpires,
    })
    .onConflictDoUpdate({
      target: waOnboardingSessionsTable.phone,
      set: {
        step: "INVITE_LGPD",
        data: { invite_token: normalizedToken, name: member.name },
        lgpd_accepted: false,
        ...
        magic_token_claimed_at: null,
        expires_at: sessionExpires,
      },
    });

  log.info({ phone: phoneNorm, tokenPrefix: normalizedToken.slice(0, 9), memberId: invite.member_id }, "wa-invite: INVITE_LGPD session opened");
  return { kind: "wa_onboarding", phone: phoneRaw, reply: replyInviteLgpd(member.name) };
}

/**
 * Step 2: invitee sends ACEITO from INVITE_LGPD → advance to INVITE_NAME.
 */
async function handleInviteLgpdStep(
  phoneRaw: string,
  phoneNorm: string,
  upper: string,
  session: { id: number; data: unknown },
  log: Logger,
): Promise<WaOnboardingOutcome> {
  if (upper !== "ACEITO") {
    const data = (session.data ?? {}) as WaOnboardingData;
    return { kind: "wa_onboarding", phone: phoneRaw, reply: replyInviteLgpd(data.name ?? "convidado") };
  }
  const data = (session.data ?? {}) as WaOnboardingData;
  await db
    .update(waOnboardingSessionsTable)
    .set({ lgpd_accepted: true, step: "INVITE_NAME" })
    .where(eq(waOnboardingSessionsTable.id, session.id));
  log.info({ phone: phoneNorm }, "wa-invite: LGPD accepted, moved to INVITE_NAME");
  return { kind: "wa_onboarding", phone: phoneRaw, reply: replyInviteAskName(data.name ?? "") };
}

/**
 * Step 3: invitee confirms/edits their name from INVITE_NAME.
 * Atomically claims the invite token (UPDATE…RETURNING), creates/upserts the
 * user, links phone + user_id to the member record, and issues a magic link.
 */
async function handleInviteNameStep(
  phoneRaw: string,
  phoneNorm: string,
  input: string,
  upper: string,
  ...
  // If user says OK/SIM/CONFIRMAR keep the suggested name, otherwise use their input.
  const isConfirmation = /^(ok|sim|confirmar|confirmo|yes)$/i.test(upper);
  const finalName = isConfirmation || !input.trim() ? suggestedName : input.trim();

  if (!finalName) {
    return { kind: "wa_onboarding", phone: phoneRaw, reply: replyInviteAskName(suggestedName) };
  }

  // Atomically claim the invite — if another concurrent request already used it,
  // no rows are returned and we abort cleanly.
  const [claimedInvite] = await db
    .update(memberInvitesTable)
    .set({ used_at: new Date() })
    .where(and(
      eq(memberInvitesTable.token, inviteToken),
      isNull(memberInvitesTable.used_at),
      gt(memberInvitesTable.expires_at, new Date()),
    ))
    .returning();
  ...
  if (!claimedInvite) {
    log.warn({ phone: phoneNorm, tokenPrefix: inviteToken.slice(0, 9) }, "wa-invite: token already used or expired at claim time");
    // Clear the stale invite session so the user can start over.
    await db.delete(waOnboardingSessionsTable).where(eq(waOnboardingSessionsTable.id, session.id));
    return {
      kind: "wa_onboarding",
      phone: phoneRaw,
      reply:
        "⚠️ Seu código de convite expirou ou já foi usado.\n\n" +
        "Peça ao administrador da família que gere um novo convite.",
    };
  }

  // Upsert user and link to household + member.
  const [user] = await db
    .insert(usersTable)
    .values({ phone: phoneNorm, firstName: finalName })
    .onConflictDoUpdate({
      target: usersTable.phone,
      set: { firstName: finalName },
    })
    .returning({ id: usersTable.id });

  if (!user) {
    log.error({ phone: phoneNorm }, "wa-invite: user upsert returned no row");
    return { kind: "wa_onboarding", phone: phoneRaw, reply: "⚠️ Ocorreu um erro. Por favor, tente novamente." };
  }

  await db.transaction(async (tx) => {
    await tx
      .update(usersTable)
      .set({ household_id: claimedInvite.household_id })
  ...
  // Issue magic link and mark session COMPLETE.
  const magicToken = generateMagicToken();
  ...
  log.info(
    { memberId: claimedInvite.member_id, householdId: claimedInvite.household_id, phone: phoneNorm },
    "wa-invite: COMPLETE — phone linked and user created",
  );

  return { kind: "wa_onboarding", phone: phoneRaw, reply: replyInviteComplete(finalName, magicToken, domain) };
}
...
export async function handleWaOnboarding(
  ...
  // ── Member invite token intercept ─────────────────────────────────────────
  // Alphanumeric VESTA-XXXXXX tokens (containing letters) are member WA invites.
  // Pure-digit tokens are caught earlier in processInboundWAMessage by looksLikeToken.
  if (looksLikeMemberInviteToken(bodyText.trim())) {
    return await startMemberInviteFlow(phoneRaw, phoneNorm, bodyText.trim(), log);
  }
  ...
  // INVITE_LGPD: abbreviated onboarding step 2 — LGPD consent for invited member.
  // Handled before the lgpd_accepted guard because the user hasn't accepted yet.
  if (currentStep === "INVITE_LGPD") {
    return await handleInviteLgpdStep(phoneRaw, phoneNorm, upper, session, log);
  }
  ...
  // INVITE_NAME: abbreviated onboarding step 3 — name confirm + atomic invite claim.
  if (currentStep === "INVITE_NAME") {
    return await handleInviteNameStep(phoneRaw, phoneNorm, input, upper, session, domain, log);
  }
